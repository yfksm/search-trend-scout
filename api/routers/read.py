...
from models.domain import Item, Tag, UserState, Source, LaneType
from models.schemas import FeedResponse, ItemBase, ItemDetail, TagSchema

router = APIRouter()

def build_query(base_query, range_days: Optional[int], lane: Optional[str], tag_id_or_name: Optional[str], q: Optional[str]):
    query = base_query
...
async def get_feed(
    range: Optional[int] = Query(7, description="Days to go back"),
    ...
    base_q = select(Item).options(selectinload(Item.source), selectinload(Item.tags)).filter(Item.id.notin_(hidden_subq))
    ...
    return FeedResponse(
        items=enhance_items_with_state(items, states),
        total=total,
        page=page,
        size=size
    )
...
async def get_bookmarks(db: AsyncSession = Depends(get_db)):
    q = select(Item).join(UserState, Item.id == UserState.item_id)\
