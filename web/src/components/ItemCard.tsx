...
export default function ItemCard({ item, mutate }: { item: any; mutate?: () => void }) {
    ...
    const currentLaneColor = item.lane ? laneColors[item.lane] : 'bg-gray-500/10 text-gray-400 border-gray-500/20';

    return (
        <div className={`relative flex flex-col glass-panel hover-lift p-5 rounded-2xl group transition-all duration-300 ${isRead ? 'bg-card/40' : 'bg-card/80'}`}>

            {/* Header Info */}
            <div className="flex items-center justify-between mb-3 text-sm">
                <div className="flex items-center gap-3">
                    <span className={`px-2.5 py-1 text-xs font-semibold uppercase tracking-wider rounded-lg border ${currentLaneColor}`}>
                        {item.lane || 'Uncategorized'}
                    </span>
                    <span className="text-slate-300 font-medium">{item.site || 'Unknown Source'}</span>
            ...
            <div className="flex-1">
                <Link href={`/items/${item.id}`} onClick={handleRead}>
                    <h2 className={`text-xl font-bold leading-tight mb-2 group-hover:text-primary transition-colors cursor-pointer ${isRead ? 'text-slate-300' : 'text-foreground'}`}>
                        {item.title}
                    </h2>
                </Link>
            ...
            {item.tags?.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                    {item.tags.slice(0, 4).map((tag: any) => (
                        <span key={tag.id} className="text-xs px-2 py-1 rounded bg-border/50 text-muted">
                            #{tag.name}
                        </span>
                    ))}
                    {item.tags.length > 4 && (
                        <span className="text-xs px-2 py-1 text-muted">+{item.tags.length - 4}</span>
                    )}
                </div>
            )}
