...
export default function ItemDetail() {
  ...
  const {
    ...
    error,
    mutate,
  } = useSWR(id ? `/api/items/${id}` : null, () => fetchItemDetail(id));
  ...
  if (error)
    return <div className="p-8 text-red-400">Error loading details.</div>;
  if (!item)
    return <div className="p-8 animate-pulse text-muted">Loading...</div>;
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <header className="space-y-4">
        ...
        <h1 className="text-4xl font-extrabold leading-tight">{item.title}</h1>
